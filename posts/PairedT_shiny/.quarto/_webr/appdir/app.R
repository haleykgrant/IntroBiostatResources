...
server <- function(input, output, session) {
  ...
  output$plot_p1 <- renderPlot({
    ...
    df <- rbind(
      data.frame(x = x_range, y = dnorm(x_range, mu1, sd1), dist = "ȳ1"),
      data.frame(x = x_range, y = dnorm(x_range, mu2, sd2), dist = "ȳ2")
    )
    
    df.x1 <- df %>% filter(dist == "ȳ1")
    df.x2 <- df %>% filter(dist == "ȳ2")
  ...
  output$plot_p2 <- renderPlot({
    ...
    ggplot() +
      ...
      theme_minimal(base_size = 14) +
      labs(x = "ȳ₁ - ȳ₂", y = "Count", title = paste0("Correlation: ", cr))
  })
  ...
  output$vars <- renderTable({
    data.frame(
      Distribution = c("ȳ1", "ȳ2", "ȳ1 - ȳ2"),
