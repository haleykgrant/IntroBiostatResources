...
server <- function(input, output) {
  ...
  set.seed(23)
  N = 100
  x1 = runif(n = N, max = 5) %>% round()
  y = 4.2 + 1.5*x1 + rnorm(n = N, sd = 1.3)
  origx2<-(3 + 0.5*x1 +rnorm(n = N, sd =1.2))  %>% plyr::round_any(.,0.5)
  ...
  output$dataPlot2 <- renderPlot({
   ...
   vals$dt %>% 
    ...
    geom_segment(aes(x = mean_x - 0.5, xend = mean_x + 0.5, y = mean_y, yend = mean_y, color = x2), linetype = "dashed") +
