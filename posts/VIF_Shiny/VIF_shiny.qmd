...
server <- function(input, output) {
  ...
  output$dataPlot2 <- renderPlot({
    ...
    vals$dt %>% 
      ...
      geom_smooth(method = "lm", se = F, size = 0.5) + 
      labs(color = "x2\n(rounded)")+  
      stat_summary(aes(x = x), fun = mean, geom = "vline", linetype = "dashed", color = "black", size = 1) +
  stat_summary(aes(y = y), fun = mean, geom = "hline", linetype = "dashed", color = "black", size = 1)
  })
