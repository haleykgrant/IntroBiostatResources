...
ui <- page_sidebar(
 ...
 sidebar = sidebar(
    ...
    radioButtons("test_type", "Test Type:", choices = list("One-sided" = "one", "Two-sided" = "two"),
                selected = "two"),
  ...
  observeEvent(input$draw, {
    ...
    output$densityPlot <- renderPlot({
      ...
      p <- ggplot(data.frame(x = plot_data$x_limits_null), aes(x)) +
        ...
        geom_text(aes(x = plot_data$t_stat, label = paste("T =", round(plot_data$t_stat, 2)), y = 0), 
                  color = "springgreen4", angle = 90, vjust = 0, hjust = 0, size = 4.5) +
      ...
      # Add the alternative distribution if showAlt is toggled on
      if (showAlt()) {
        p <- p + stat_function(fun = dlst, args = list(mu = (input$mu_true-input$mu0) / (plot_data$sample_sd / sqrt(input$n)), 
                                                       df = input$n - 1),
                               color = "lightblue", alpha = 0.7) + 
          labs(title = "Test Statistics: Null vs Alternative")
      }
    ...
    output$meanDensityPlot <- renderPlot({
      ...
      p <- ggplot(data.frame(x = plot_data$mean_x_limits), aes(x)) + 
        ...
        geom_text(aes(x = plot_data$sample_mean, label = paste("X̄ =", round(plot_data$sample_mean, 2)), y = 0), 
                  color = "orange3", angle = 90, vjust = 0, hjust = 0, size = 4.5) +
